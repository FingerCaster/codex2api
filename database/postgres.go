...
func (db *DB) migrate(ctx context.Context) error {
	...
	ALTER TABLE system_settings ADD COLUMN IF NOT EXISTS disable_v1_messages BOOLEAN DEFAULT FALSE;
...
const apiKeySelectColumns = `id, name, key, COALESCE(enabled, true), created_at, COALESCE(quota_limit, 0), COALESCE(quota_used, 0), expires_at, COALESCE(allowed_group_ids, '[]')`
...
func (db *DB) GetSystemSettings(ctx context.Context) (*SystemSettings, error) {
	...
	err := db.conn.QueryRowContext(ctx, `
		SELECT COALESCE(site_name, 'CodexProxy'), COALESCE(site_logo, ''),
		       ...
		       COALESCE(disable_v1_messages, false)
	...
	`).Scan(
		...
		&s.ImageStorageConfig, &s.DisableV1Messages,
	)
...
func (db *DB) UpdateSystemSettings(ctx context.Context, s *SystemSettings) error {
	_, err := db.conn.ExecContext(ctx, `
			INSERT INTO system_settings (
				...
				image_storage_config, disable_v1_messages
			)
			...
			auto_clean_rate_limited = EXCLUDED.auto_clean_rate_limited,
				...
				disable_v1_messages = EXCLUDED.disable_v1_messages
...
func (db *DB) GetUsageStatsByAPIKey(ctx context.Context, apiKeyID *int64) (*UsageStats, error) {
	...
	if apiKeyID != nil {
		bReq, bTok, bPrompt, bComp, bCached, bCacheHitRequests, bFirstTokenSamples = 0, 0, 0, 0, 0, 0, 0
		bFirstTokenMsSum, bAccountBilled, bUserBilled = 0, 0, 0
	}
...
func (db *DB) buildUsageLogWhere(f UsageLogFilter) (string, []interface{}) {
	...
	if !f.Start.IsZero() {
		p := addArg(db.timeArg(f.Start))
		parts = append(parts, fmt.Sprintf(`u.created_at >= %s`, p))
	}
	if !f.End.IsZero() {
		p := addArg(db.timeArg(f.End))
		parts = append(parts, fmt.Sprintf(`u.created_at <= %s`, p))
	}
...
func (db *DB) ListActive(ctx context.Context) ([]*AccountRow, error) {
	query := `
		SELECT id, name, platform, type, credentials, proxy_url, status, cooldown_reason, cooldown_until, error_message, COALESCE(disabled, false), COALESCE(enabled, true), COALESCE(locked, false), score_bias_override, base_concurrency_override, COALESCE(tags, '[]'), created_at, updated_at
		FROM accounts
		WHERE status <> 'deleted' AND COALESCE(error_message, '') <> 'deleted'
		ORDER BY id
	`
...
func (db *DB) SetAccountDisabled(ctx context.Context, id int64, disabled bool) error {
	res, err := db.conn.ExecContext(ctx, `UPDATE accounts SET disabled = $1, enabled = $2, updated_at = CURRENT_TIMESTAMP WHERE id = $3`, disabled, !disabled, id)
	if err != nil {
		return err
	}
	affected, err := res.RowsAffected()
...
func (db *DB) SetAccountEnabled(ctx context.Context, id int64, enabled bool) error {
	res, err := db.conn.ExecContext(ctx, `UPDATE accounts SET enabled = $1, disabled = $2, updated_at = CURRENT_TIMESTAMP WHERE id = $3`, enabled, !enabled, id)
	if err != nil {
		return err
	}
	affected, err := res.RowsAffected()
