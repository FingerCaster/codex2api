...
func (db *DB) GetUsageStatsByAPIKey(ctx context.Context, apiKeyID *int64) (*UsageStats, error) {
	...
	err = db.conn.QueryRowContext(ctx, `
			...
			FROM usage_logs
			WHERE status_code <> 499
		`, totalArgs...).Scan(&stats.TotalRequests, &currentAccountBilled, &currentUserBilled)
	if err != nil {
		return nil, err
	}
	...
	if apiKeyID == nil {
		...
		stats.TotalAccountBilled = currentAccountBilled
		stats.TotalUserBilled = currentUserBilled
	}
...
func (db *DB) buildUsageLogWhereClause(f UsageLogFilter) (string, []interface{}, int) {
	...
	if !f.Start.IsZero() {
		...
		args = append(args, db.timeArg(f.Start))
		paramIdx++
	}
	if !f.End.IsZero() {
		conditions = append(conditions, fmt.Sprintf(`u.created_at <= $%d`, paramIdx))
		args = append(args, db.timeArg(f.End))
		paramIdx++
	}
	if f.Email != "" {
	...
	if f.Model != "" {
		conditions = append(conditions, fmt.Sprintf(`(u.model = $%d OR COALESCE(u.effective_model, '') = $%d)`, paramIdx, paramIdx))
...
func (db *DB) ListActive(ctx context.Context) ([]*AccountRow, error) {
	query := `
		SELECT id, name, platform, type, credentials, proxy_url, status, cooldown_reason, cooldown_until, error_message, COALESCE(disabled, false), COALESCE(enabled, true), COALESCE(locked, false), score_bias_override, base_concurrency_override, created_at, updated_at
		FROM accounts
		WHERE status <> 'deleted' AND COALESCE(error_message, '') <> 'deleted'
		ORDER BY id
	`
